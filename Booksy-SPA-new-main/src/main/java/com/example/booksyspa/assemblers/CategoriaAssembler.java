...
public class CategoriaAssembler implements RepresentationModelAssembler<Categoria, EntityModel<Categoria>> {
    ...
    public EntityModel<Categoria> toModel(Categoria categoria) {
        Link selfLink = linkTo(methodOn(CategoriaController.class).getCategoriaById(categoria.getIdCategoria())).withSelfRel();
        Link librosLink = linkTo(methodOn(LibroController.class).getLibrosByCategoria(categoria.getIdCategoria())).withRel("libros-de-la-categoria");
        return EntityModel.of(categoria, selfLink, librosLink);
    }
}
